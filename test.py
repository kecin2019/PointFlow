...
def evaluate_gen(model, args):
    ...
    save_dir = os.path.dirname(args.resume_checkpoint)
    np.save(os.path.join(save_dir, "model_out_smp.npy"), sample_pcl_npy)
    np.save(os.path.join(save_dir, "model_out_ref.npy"), ref_pcl_npy)

    os.makedirs(os.path.join(save_dir, "point_cloud"), exist_ok=True)
    # Save the point clouds as text files
    for i in range(sample_pcl_npy.shape[0]):
        filename = "sample_pcs_%03d.txt" % i
        np.savetxt(
            os.path.join(save_dir, "point_cloud", filename), sample_pcl_npy[i], fmt="%s"
        )
    for i in range(ref_pcl_npy.shape[0]):
        filename = "ref_pcs_%03d.txt" % i
        np.savetxt(
            os.path.join(save_dir, "point_cloud", filename), ref_pcl_npy[i], fmt="%s"
        )
