...
from datasets_grape import get_datasets, init_np_seed
import imageio
...
def main_worker(gpu, save_dir, ngpus_per_node, args):
    ...
    if not args.distributed or (args.rank % ngpus_per_node == 0):
        np.save(
            os.path.join(save_dir, "train_set_mean.npy"), tr_dataset.all_points_mean
        )
        np.save(os.path.join(save_dir, "train_set_std.npy"), tr_dataset.all_points_std)
        np.save(
            os.path.join(save_dir, "train_set_idx.npy"),
        ...
        np.save(
            os.path.join(save_dir, "val_set_idx.npy"), np.array(te_dataset.shuffle_idx)
        )
        ...
        if (epoch + 1) % args.viz_freq == 0:
            ...
            imageio.imwrite(
                os.path.join(
                    save_dir,
                    "images",
                    "tr_vis_conditioned_epoch%d-gpu%s.png" % (epoch, args.gpu),
                ),
                res.transpose((1, 2, 0)),
            )
            if writer is not None:
